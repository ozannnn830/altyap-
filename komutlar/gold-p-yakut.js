  ...
  Jimp.read(`https://cdn.discordapp.com/attachments/649581228835602432/650052923728068628/unknown.png`, async(err, image) => {
    ...
    Jimp.read(user.avatarURL(), async(err, avatar) => {
      ...
      await image.composite(avatar, 269, 39).write(`./resimler/gold/gold-${message.author.id}.png`);
        setTimeout(function() {
            message.reply(`Bravo! **Yakut Kasadan! (${kod31})** Gün Gold Üye Kazandın Azim İle Yapamıyacağın Pek Birşey Yok Sanırım Gold Üyeni Verdim Artık Kullanabilirsin Hayırlı Olsun.`)
            return message.channel.send(new Discord.MessageAttachment(`./resimler/gold/gold-${message.author.id}.png`));
        }, 1000);
      });
    return
    });
...
module.exports.conf = {
  enabled: true,
  guildOnly: false,
  aliases: ['yakut', "yakutkasa", "yakutkasa"],
  permLevel: 0
};
